...
#     involvement. Needed for the AKS track — the shared cluster lives outside
#     this VNet with no peering (see terraform-shared-aks/main.tf's network
#     note: peering would need a manual trainer-side grant per learner, rights
#     this apply's own identity doesn't have on the trainer's shared RG,
#     same as it can't grant itself the AKS namespace RBAC role either).
#     Credentials + TLS become the real boundary here, same trade-off already
#     made for Redis (redis.tf) and backend<->frontend CORS
#     (app-service-java.tf).
#
