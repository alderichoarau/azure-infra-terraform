...
# case, see the module) means nobody gets in with an account key, only with an
# Azure AD identity holding a role on this specific container — and only the
# Java backend's managed identity gets one, below. Same category of trade-off
# as app-service-java.tf's CORS + API key for the same underlying reason
# (an existing shared resource whose other tenants can't be touched).
# ──────────────────────────────────────────────────────────────────────────────
