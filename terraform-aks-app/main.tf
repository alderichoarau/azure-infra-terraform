# ──────────────────────────────────────────────────────────────────────────────
# main.tf — TP Java/Angular, piste "AKS".
#
...
# itself (data.azurerm_resource_group.rg: "pre-created by the trainer, never
# managed by Terraform"). The trainer runs scripts/bootstrap-aks-namespace.sh
# once per learner instead — see that script and ../terraform-shared-aks/README.md
# for the full reasoning (a shared multi-tenant cluster fundamentally needs
# one trust-establishing step from whoever owns it, same as RG pre-creation
# already does for everything else in this repo).
#
# Deliberately its own directory/state, separate from
# ../terraform-managed-services even though the AKS-track backend connects to
# that directory's Postgres/Redis — see that directory's README for why they
# stay independent despite the resource-sharing.
# ──────────────────────────────────────────────────────────────────────────────
...
# convention (aks-<environment>-<cohort>) must match that directory's
...
# (storage_use_azuread, ../terraform-core/providers.tf) and Key Vault (RBAC,
# not access policies) elsewhere in this repo: push/pull goes through Azure
# AD identities, never a shared admin username/password.
...
# ci_app_deploy (../terraform-core) also needs to push images to this ACR
# from azure-quiz-backend/frontend's deploy-aks.yml — AcrPush, same
# self-service scope.
resource "azurerm_role_assignment" "ci_app_deploy_acr_push" {
  scope                = azurerm_container_registry.app.id
  role_definition_name = "AcrPush"
  principal_id         = data.terraform_remote_state.core.outputs.ci_app_deploy_principal_id
}
