...
variable "runner_admin_ip_cidr" {
  description = "CIDR allowed to SSH (22) into the runner VM -- the owner's own IP, not a shared training room's."
  type        = string

  validation {
    condition     = can(cidrhost(var.runner_admin_ip_cidr, 0))
    error_message = "runner_admin_ip_cidr must be a valid CIDR block, e.g. \"203.0.113.4/32\"."
  }
}

variable "runner_vm_size" {
  description = "VM size for the runner VM. Standard_B2s default (2 vCPU/4GB, burstable, cheapest fit for lint/test/build workloads) -- if this errors with a capacity restriction (B-series has been capacity-restricted on this owner's other subscription in francecentral, see ../terraform-prometheus), fall back to Standard_D2s_v3."
  type        = string
