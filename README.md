...
| Azure Managed Redis | `redis-<owner>-tf` | TP Java/Angular. Replaces the retired Azure Cache for Redis. No public access, reachable only via its Private Endpoint on `subnet-backend`. `access_keys_authentication_enabled = true` so the backend can authenticate (TLS required, `client_protocol` defaults to `Encrypted`) — the access key is pushed to Key Vault (`redis-access-key` secret), never output in the clear — see [redis.tf](terraform/redis.tf) |
