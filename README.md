...
**TLS**: `scripts/setup-cert-manager.sh` (trainer-run, one-shot per cluster) installs cert-manager + two Let's Encrypt `ClusterIssuer`s (staging/prod), so those `nip.io` Ingress hosts get a real, browser-trusted cert instead of the App Routing add-on's self-signed default — both Helm charts opt in via `ingress.clusterIssuer` in `values.yaml`.
...
| **Setup cert-manager** (`.github/workflows/setup-cert-manager.yml`) | Manual, trainer-only, one-shot per cluster | Installs cert-manager + Let's Encrypt `ClusterIssuer`s so AKS Ingress hosts get a real TLS cert instead of the self-signed default |
