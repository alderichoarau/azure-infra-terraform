...
# (ci_app_deploy, ../terraform-core) already has Contributor on the whole
# Resource Group, but that's not enough on its own: Contributor is
# control-plane only (create/delete/configure resources) — it grants zero
# DataActions on any service, so it can't read a secret's *value* from an
# RBAC-authorized vault any more than Contributor alone lets you read blob
# contents on the shared Storage Account (see ../terraform-core's own access
# model). Needs this explicit data-plane role on top.
