# ──────────────────────────────────────────────────────────────────────────────
# app-service-java.tf — Java backend, on the shared App Service Plan
...
# it is mutualised, exactly like Python's setup.
# ──────────────────────────────────────────────────────────────────────────────

# subnet-backend (../terraform-core, 10.0.2.0/24) can't be reused here: it
# comes from the shared network registry module with no delegation, and it
# already hosts the Redis/Key Vault Private Endpoints (redis.tf, keyvault.tf).
# Web App regional VNet Integration requires its subnet delegated to
...
# gets its own subnet rather than touching the core registry module.
...
resource "azurerm_linux_web_app" "java_app" {
  ...
  site_config {
    ...
    # CORS. With a single instance (no autoscale configured on the shared
    # plan) this doesn't yet drive load-balancer routing, but it does make
    # App Service auto-restart this app's instance if health checks fail for
    # the eviction window below, instead of silently serving from a hung
    # process.
