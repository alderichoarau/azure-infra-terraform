...
# + Availability Tests + alerts, wired to app-service.tf / function-app.tf /
# ../terraform-core's shared Storage Account.
...
# storage_account_id comes from ../terraform-core (module.storage_shared lives
# there, not in this directory) — this diagnostic setting is the one place
# this track reaches back into core's remote state for something that isn't
# the network.
resource "azurerm_monitor_diagnostic_setting" "storage_blob" {
  name                       = "diag-storage-blob-${var.owner}"
  target_resource_id         = "${data.terraform_remote_state.core.outputs.storage_account_id}/blobServices/default"
  log_analytics_workspace_id = azurerm_log_analytics_workspace.law.id
